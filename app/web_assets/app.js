...
function renderLeadCabinet(){
  ...
  const cards=state.leadBanks.map(item=>`<label class="client-bank-card ${canSelect?'is-selectable':''}">${canSelect?`<input class="bank-choice" type="checkbox" value="${esc(item.bank_id)}">`:''}<header><span class="client-bank-icon">${esc(initials(item.bank).slice(0,1)||'Б')}</span><div><h3>${esc(item.bank)}</h3><p>${canSelect?'Доступен для открытия':esc(bankLabels[item.status]||item.status)}</p></div><strong class="bank-payout">${esc(item.payout_text||'Уточняется')}</strong>${canSelect?'<span class="choice-mark">✓</span>':''}</header><section class="activation-action"><span>Условие активации</span><p>${esc(item.action_text||'Условие уточняется')}</p></section><small>Обновлено ${dateTime(item.updated)}</small></label>`).join('')
