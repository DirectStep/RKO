...
function renderLeadCabinet(){
  ...
  const cards=state.leadBanks.map(item=>{const selectable=item.selected===null||(state.leadAddingBanks&&item.selected===false);const dimmed=item.selected===false&&!state.leadAddingBanks;const status=selectable?'Доступен для открытия':item.selected===false?'Не выбран':item.lead_payment_status==='paid'?bankLabels.lead_reward_paid:(bankLabels[item.status]||item.status);const statusTone=item.status==='opened'?'is-positive':['not_opened','will_not_open'].includes(item.status)?'is-negative':'';return `<label class="client-bank-card ${selectable?'is-selectable':''} ${dimmed?'is-unselected':''}">${selectable?`<input class="bank-choice" type="checkbox" value="${esc(item.bank_id)}">`:''}${item.online_available?`<span class="online-badge">Можно онлайн <button type="button" data-online-help="${esc(item.online_help)}" aria-label="Условия открытия онлайн">${infoIcon}</button></span>`:''}<header><span class="client-bank-icon">${esc(initials(item.bank).slice(0,1)||'Б')}</span><div class="client-bank-copy"><h3>${esc(item.bank)}</h3><p class="${statusTone}">${esc(status)}</p></div>${selectable?'<span class="choice-mark">✓</span>':''}<strong class="bank-payout">${leadPayout(item)}</strong></header><section class="activation-action"><span>Условие активации</span><p>${esc(item.action_text||'Условие уточняется')}</p></section><small>Обновлено ${dateTime(item.updated)}</small></label>`}).join('')
