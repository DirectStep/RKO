...
const applicationLabels = { questionnaire_completed:'Анкета заполнена',questionnaire_ineligible:'Анкета не подходит',banks_selected:'Банки выбраны',opening_accounts:'Оформление счетов',activating_accounts:'Активация счетов',completed:'Завершена' }
...
const bankLabels = { planned:'Планируем открыть',awaiting_activation:'Счёт открыт (ждёт активации)',awaiting_data:'Ждём данные от клиента',preparing_application:'Готовим заявку в банк',application_sent:'Заявка отправлена в банк',under_review:'Банк рассматривает заявку',revision_required:'Нужно дополнить данные',account_opened:'Счёт активирован (холд)',account_activated:'Счёт активирован',bank_rejected:'Отказ банка',client_refused:'Отказ клиента',cut:'Срез',duplicate:'Дубль',excluded:'Банк исключён',in_progress:'Открытие в работе',opened:'Счёт активирован (холд)',not_opened:'Счёт не открыт',will_not_open:'Открывать не будем',lead_reward_paid:'Выплата сделана' }
...
function showBonusHelp(bankName){
  const alpha=['Альфа-Банк (ИП+счёт)','Альфа-Банк (без открытия ИП)'].includes(bankName)
  const more=alpha?'<p><a href="https://alfabank.ru/help/t/corp/korporativnie-debetovie-karti/alfa-biznes/keshbek/kak-konvertirovat-balli-v-rubli?ysclid=muhl3mhe3665457855" target="_blank" rel="noopener">Подробнее тут</a></p>':''
  openSheet('Бонусы за задания',bankName,`<section class="detail-section online-help-content"><p class="info-copy">Выполняйте задания и получайте бонусы от банка.</p><p class="info-copy">1 бонус = 1 ₽</p>${more}</section><button class="primary-button" id="close-bonus-help">Понятно</button>`)
  document.querySelector('#close-bonus-help').addEventListener('click',closeSheet)
}
...
function filteredLeads(){
  ...
  const partner=document.querySelector('#admin-partner').value,channel=document.querySelector('#admin-channel').value,status=document.querySelector('#admin-lead-status').value,period=document.querySelector('#admin-period').value
  ...
  if(status)items=items.filter(lead=>lead.application_status===status)
  let from='',to=''
...
function renderLeadCabinet(){
  ...
  document.querySelector('#client-application-card').innerHTML=`<section class="client-hero"><span>Что сейчас с заявкой</span><strong>${esc(applicationLabels[application.application_status]||application.application_status)}</strong><small>Обновлено ${dateTime(application.updated)}</small></section><section class="client-metric-grid"><div><span>Планируется счетов</span><strong>${metrics.planned_accounts||0}</strong></div><div><span>Активированных счетов</span><strong>${metrics.activated_accounts||0}</strong></div><div><span>Ожидаемая выплата</span><strong>${money(metrics.expected_payout)}</strong></div><div><span>Выплачено всего</span><strong>${money(metrics.paid_total)}</strong></div></section><section class="detail-section"><h3>Данные заявки</h3><div class="value-row"><span>Подана</span><strong>${date(application.date)}</strong></div>${admin}${manager}</section><p class="client-note">Здесь всегда виден текущий этап заявки.</p>`
  const cards=state.leadBanks.map(item=>{
    const selectable=item.selected===null||(state.leadAddingBanks&&item.selected===false)
    const dimmed=item.selected===false&&!state.leadAddingBanks
    const displayStatus=item.display_status||item.status
    const reoffered=item.selected===false&&state.leadAddingBanks&&['not_opened','bank_rejected','client_refused'].includes(displayStatus)
    const status=item.selected===null?'Доступен для открытия':reoffered?'Доступен для повторного выбора':item.selected===false&&displayStatus==='planned'?'Не выбран':bankLabels[displayStatus]||displayStatus
...
function bankCard(item,employee,admin,quickActions){
  const financialEditable=admin&&employee&&!['not_opened','bank_rejected','client_refused','cut','duplicate'].includes(item.status)
  const financialEdit=financialEditable?`<div class="field-row"><label class="field"><span>Общая ставка, прогноз</span><input type="number" data-estimate value="${item.income_estimate||''}"></label><label class="field"><span>Общая ставка, факт</span><input type="number" data-fact value="${item.income_fact||''}" ${item.status==='account_opened'&&!['confirmed','in_registry','paid'].includes(item.payment_status)?'':'disabled'}></label></div>`:''
  const decisionStatus=bankLabels[item.display_status||item.status]||item.status
  const decisionView=`<div class="value-row"><span>Статус</span><strong>${esc(decisionStatus)}</strong></div>${item.close_reason?`<div class="value-row"><span>Причина</span><strong>${esc(item.close_reason)}</strong></div>`:''}`
  const adminEdit=`${decisionView}${financialEdit}${financialEditable?`<button class="secondary-button" data-save-bank="${item.id}">Сохранить финансовые данные</button>`:''}`
  const edit=admin?adminEdit:employee?decisionView:`<div class="value-row"><span>Статус</span><strong>${esc(decisionStatus)}</strong></div>`
  const canClose= !item.lead_reward_paid_at&&!['confirmed','in_registry','paid'].includes(item.payment_status)
  const quick=quickActions&&item.selected_by_lead===true&&item.status==='planned'?`<div class="bank-quick-actions"><button class="primary-button" data-bank-awaiting="${item.id}">Счёт открыт (ждёт активации)</button><button class="danger-button" data-bank-not-opened="${item.id}" data-bank-name="${esc(item.bank)}">Счёт не открыт</button></div>`:quickActions&&item.selected_by_lead===true&&item.status==='awaiting_activation'?`<div class="bank-quick-actions"><button class="primary-button" data-bank-opened="${item.id}">Счёт активирован (холд)</button><button class="danger-button" data-bank-reject="${item.id}" data-bank-name="${esc(item.bank)}">Отказ</button></div>`:quickActions&&item.status==='account_opened'&&canClose?`<div class="bank-quick-actions"><button class="danger-button" data-bank-cut="${item.id}" data-bank-name="${esc(item.bank)}">Срез</button><button class="danger-button" data-bank-duplicate="${item.id}" data-bank-name="${esc(item.bank)}">Дубль</button></div>`:quickActions&&['not_opened','bank_rejected','client_refused'].includes(item.status)&&item.selected_by_lead===true?`<button class="secondary-button" data-bank-reoffer="${item.id}" data-reoffer="true">Вернуть в доступные для лида</button>`:quickActions&&['not_opened','bank_rejected','client_refused'].includes(item.status)&&item.selected_by_lead===false?`<button class="secondary-button" data-bank-reoffer="${item.id}" data-reoffer="false">Убрать из доступных для лида</button>`:''
  const confirm=admin&&employee&&(item.lead_reward_paid_at||item.lead_reward_paid_separately)&&['awaiting_confirmation','confirmed','in_registry'].includes(item.payment_status)&&item.reward_fact!==null?`<button class="primary-button" data-confirm-pay="${item.id}">Подтвердить выплату</button>`:''
  ...
  const partnerPaid=item.payment_status==='paid'
  const partnerActual=item.reward_fact!==null&&item.reward_fact!==undefined
  const teamActual=item.team_profit_fact!==null&&item.team_profit_fact!==undefined
...
async function openLead(id){
  ...
  try{
    ...
    const canManageBanks=editable&&admin&&lead.is_primary_admin
    const username=String(lead.username||'').replace(/^@/,'')
    const telegramLink=/^[A-Za-z0-9_]{5,}$/.test(username)?`https://t.me/${username}`:''
    const extraContacts=(managerRole||admin)&&lead.application_status!=='questionnaire_ineligible'?`<div class="value-row"><span>Город</span><strong>${esc(lead.answers?.city||'Не указан')}</strong></div><label class="field inset-field"><span>Улица и номер дома</span><input id="lead-street-address" maxlength="300" placeholder="запросить информацию у лида" value="${esc(lead.street_address||'')}" ${lead.archived?'disabled':''}></label><label class="field inset-field"><span>ИНН</span><input id="lead-inn-draft" maxlength="120" placeholder="запросить информацию у лида" value="${esc(lead.inn_draft||'')}" ${lead.archived?'disabled':''}></label>${lead.archived?'':'<button class="secondary-button inset-button" id="save-lead-contacts">Сохранить данные</button>'}`:''
    const contacts=partner?'':`<section class="detail-section"><h3>Контакты</h3><button type="button" class="contact-row" data-copy-contact="${encodeURIComponent(String(lead.phone||''))}"><span><small>Телефон</small><strong>${esc(lead.phone||'Не указан')}</strong></span><b>Скопировать</b></button>${lead.email?`<button type="button" class="contact-row" data-copy-contact="${encodeURIComponent(String(lead.email))}"><span><small>E-mail</small><strong>${esc(lead.email)}</strong></span><b>Скопировать</b></button>`:''}${telegramLink?`<a class="contact-row" href="${esc(telegramLink)}" target="_blank" rel="noopener"><span><small>Telegram</small><strong>@${esc(username)}</strong></span><b aria-hidden="true">Открыть</b></a>`:`<div class="value-row"><span>Telegram</span><strong>${esc(lead.username||lead.telegram_id||'Не указан')}</strong></div>`}${extraContacts}</section>`
    ...
    const hasActivatedAccounts=lead.banks.some(bank=>bank.status==='account_opened')
    const hasConfirmedPayments=lead.banks.some(bank=>bank.lead_reward_paid||['confirmed','in_registry','paid'].includes(bank.payment_status))
    const canDeleteApplication=admin&&!lead.archived&&!hasActivatedAccounts&&!hasConfirmedPayments
    const deleteApplication=canDeleteApplication?`<section class="destructive-section"><button class="danger-button" id="show-delete-lead">Удалить эту заявку</button><div id="delete-lead-confirm" hidden><p>Заявка, её банки и неподтверждённые выплаты будут удалены. Telegram-аккаунт клиента и другие его заявки останутся.</p><div class="button-stack"><button class="danger-button" id="delete-lead">Да, удалить заявку</button><button class="secondary-button" id="cancel-delete-lead">Отмена</button></div></div></section>`:''
    ...
    const workflow=`<section class="detail-section"><h3>Этап обработки</h3><div class="value-row"><span>Статус заявки</span><strong>${esc(applicationLabels[lead.application_status]||lead.application_status)}</strong></div><div class="value-row"><span>Первичный ответственный</span><strong>${esc(lead.primary_admin||'Не назначен')}</strong></div>${managerControl}${workflowActions}</section>`
    const canUseBankQuickActions=admin&&Boolean(lead.bank_selection_submitted_at)&&!lead.archived
    const banks=lead.banks.map(x=>bankCard(x,editable,admin,canUseBankQuickActions)).join('')
...
function openPartnerLead(id){
  ...
  const progress=lead.bank_progress||{}
  const banks=lead.banks.map((bank,index)=>`<button type="button" class="bank-card bank-card-button" data-partner-bank="${index}"><header><h4>${esc(bank.bank)}</h4><span>${money(Number(bank.reward_estimate)+Number(bank.reward_fact))}</span></header><small>Нажмите, чтобы посмотреть выплаты</small></button>`).join('')
  openSheet(lead.name,`${lead.short_id} · ${lead.is_repeat?'Повторная · ':''}${applicationLabels[lead.application_status]||lead.application_status}`,`<section class="detail-section"><h3>Заявка клиента</h3><div class="value-row"><span>Номер</span><strong>${esc(lead.short_id)}</strong></div><div class="value-row"><span>Дата заявки</span><strong>${dateTime(lead.date)}</strong></div><div class="value-row"><span>Последнее обновление</span><strong>${dateTime(lead.updated)}</strong></div><div class="value-row"><span>Канал</span><strong>${esc(lead.channel)}</strong></div><div class="value-row"><span>Статус заявки</span><strong>${esc(applicationLabels[lead.application_status]||lead.application_status)}</strong></div>${admin}</section><section class="detail-section"><h3>Банки и выплаты</h3><div class="value-row"><span>Открыто</span><strong>${progress.opened||0}/${progress.total||0}</strong></div><div class="value-row"><span>Активировано</span><strong>${progress.activated||0}/${progress.total||0}</strong></div><div class="value-row"><span>Ожидаемые выплаты</span><strong>${money(progress.expected_payout)}</strong></div><div class="value-row"><span>Подтверждённые выплаты</span><strong>${money(progress.confirmed_payout)}</strong></div></section>${banks||'<p class="empty">Банки пока не добавлены</p>'}`)
...
function openPartnerBank(lead,index){
  ...
  openSheet(bank.bank,'Ставки по заявке',`<section class="detail-section"><div class="value-row"><span>Статус счёта</span><strong>${esc(bankLabels[bank.display_status||bank.status]||bank.display_status||bank.status)}</strong></div><div class="value-row"><span>Ваша ожидаемая выплата</span><strong>${money(bank.reward_estimate)}</strong></div><div class="value-row"><span>Вам подтверждено</span><strong>${money(bank.reward_fact)}</strong></div><div class="value-row"><span>Выплата клиенту</span><strong>${money(bank.lead_reward_estimate)}</strong></div></section><button class="secondary-button" id="back-to-partner-lead">Назад к заявке</button>`)
...
function bindLeadActions(lead,admin){
  document.querySelector('#save-lead-contacts')?.addEventListener('click',async()=>{const button=document.querySelector('#save-lead-contacts');button.disabled=true;try{await api(`/api/leads/${lead.id}`,{method:'PATCH',body:JSON.stringify({update_contacts:true,street_address:document.querySelector('#lead-street-address').value,inn_draft:document.querySelector('#lead-inn-draft').value})});toast('Контакты сохранены');await openLead(lead.id)}catch(error){button.disabled=false;toast(error.message)}})
  ...
  document.querySelectorAll('[data-bank-awaiting]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;try{await api(`/api/lead-banks/${btn.dataset.bankAwaiting}`,{method:'PATCH',body:JSON.stringify({status:'awaiting_activation'})});toast('Счёт открыт, ждёт активации');await openLead(lead.id);await load()}catch(error){btn.disabled=false;toast(error.message)}}))
  document.querySelectorAll('[data-bank-opened]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;try{const result=await api(`/api/lead-banks/${btn.dataset.bankOpened}`,{method:'PATCH',body:JSON.stringify({status:'account_opened'})});toast(result.sheet_sync_error||'Счёт активирован (холд)');await openLead(lead.id);await load()}catch(error){btn.disabled=false;toast(error.message)}}))
  document.querySelectorAll('[data-bank-not-opened]').forEach(btn=>btn.addEventListener('click',()=>openBankOutcome(lead,btn.dataset.bankNotOpened,btn.dataset.bankName,'not_opened')))
  document.querySelectorAll('[data-bank-cut]').forEach(btn=>btn.addEventListener('click',()=>openBankOutcome(lead,btn.dataset.bankCut,btn.dataset.bankName,'cut')))
  document.querySelectorAll('[data-bank-duplicate]').forEach(btn=>btn.addEventListener('click',()=>openBankOutcome(lead,btn.dataset.bankDuplicate,btn.dataset.bankName,'duplicate')))
  document.querySelectorAll('[data-bank-reoffer]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;try{await api(`/api/lead-banks/${btn.dataset.bankReoffer}`,{method:'PATCH',body:JSON.stringify({reoffer_to_lead:btn.dataset.reoffer==='true'})});toast(btn.dataset.reoffer==='true'?'Банк снова доступен лиду':'Банк убран из доступных');await openLead(lead.id);await load()}catch(error){btn.disabled=false;toast(error.message)}}))
  ...
  document.querySelectorAll('[data-save-bank]').forEach(btn=>btn.addEventListener('click',async()=>{const c=btn.closest('[data-bank-card]'),payload={},estimate=c.querySelector('[data-estimate]'),fact=c.querySelector('[data-fact]');if(estimate)payload.income_estimate=estimate.value||null;if(fact&&!fact.disabled)payload.income_fact=fact.value||null;await api(`/api/lead-banks/${btn.dataset.saveBank}`,{method:'PATCH',body:JSON.stringify(payload)});toast('Изменения сохранены');await openLead(lead.id)}))
  document.querySelectorAll('[data-confirm-lead-payment]').forEach(btn=>btn.addEventListener('click',async()=>{const card=btn.closest('[data-bank-card]'),amount=card.querySelector('[data-lead-payment-amount]').value;if(amount===''||Number(amount)<0)return toast('Укажите фактическую сумму выплаты');btn.disabled=true;try{const result=await api(`/api/lead-banks/${btn.dataset.confirmLeadPayment}/lead-reward/confirm`,{method:'POST',body:JSON.stringify({amount})});toast(result.sheet_sync_error||'Выплата лиду сохранена');await openLead(lead.id);await load()}catch(error){btn.disabled=false;toast(error.message)}}))
  document.querySelectorAll('[data-confirm-pay]').forEach(btn=>btn.addEventListener('click',()=>{
    const bank=lead.banks.find(item=>item.id===btn.dataset.confirmPay)
    if(!bank)return
    openSheet('Подтвердить выплату',lead.short_id,`<section class="detail-section"><div class="value-row"><span>Лид</span><strong>${esc(lead.name)}</strong></div><div class="value-row"><span>Банк</span><strong>${esc(bank.bank)}</strong></div><div class="value-row"><span>Партнёру</span><strong>${money(bank.reward_fact)}</strong></div></section><p class="confirmation-note">Подтверждайте только после фактической выплаты партнёру.</p><div class="button-stack"><button class="primary-button" id="finish-partner-payment">Подтвердить выплату</button><button class="secondary-button" id="cancel-partner-payment">Отмена</button></div>`)
    document.querySelector('#cancel-partner-payment').addEventListener('click',()=>openLead(lead.id))
    document.querySelector('#finish-partner-payment').addEventListener('click',async()=>{const button=document.querySelector('#finish-partner-payment');button.disabled=true;try{const result=await api(`/api/lead-banks/${bank.id}/payment/pay`,{method:'POST',body:'{}'});toast(result.notification_pending?'Выплата сохранена, уведомление будет отправлено повторно':'Выплата партнёру подтверждена');await openLead(lead.id);await load()}catch(error){button.disabled=false;toast(error.message)}})
  }))
}
function openBankRejection(lead,bankId,bankName){
  openSheet('Оформить отказ',bankName,`<section class="form-card"><label class="field"><span>Кто отказал</span><select id="bank-rejection-type"><option value="bank_rejected">Банк</option><option value="client_refused">Клиент</option></select></label><label class="field"><span>Причина</span><textarea id="bank-rejection-reason" placeholder="Кратко опишите причину"></textarea></label><label class="check-field"><input type="checkbox" id="bank-rejection-reoffer"><span>Снова показать банк лиду в «Добавить ещё»</span></label></section><div class="button-stack"><button class="danger-button" id="confirm-bank-rejection">Сохранить отказ</button><button class="secondary-button" id="cancel-bank-rejection">Отмена</button></div>`)
  document.querySelector('#confirm-bank-rejection').addEventListener('click',async()=>{const reason=document.querySelector('#bank-rejection-reason').value.trim();if(reason.length<2)return toast('Укажите краткую причину');const button=document.querySelector('#confirm-bank-rejection');button.disabled=true;try{await api(`/api/lead-banks/${bankId}`,{method:'PATCH',body:JSON.stringify({status:document.querySelector('#bank-rejection-type').value,close_reason:reason,reoffer_to_lead:document.querySelector('#bank-rejection-reoffer').checked})});toast('Отказ сохранён');await openLead(lead.id);await load()}catch(error){button.disabled=false;toast(error.message)}})
  document.querySelector('#cancel-bank-rejection').addEventListener('click',()=>openLead(lead.id))
}
function openBankOutcome(lead,bankId,bankName,status){
  const title=bankLabels[status]
  const reoffer=status==='not_opened'?'<label class="check-field"><input type="checkbox" id="bank-reoffer"><span>Снова показать банк лиду в «Добавить ещё»</span></label>':''
  openSheet(title,bankName,`<section class="form-card"><label class="field"><span>Причина</span><textarea id="bank-outcome-reason" placeholder="Кратко опишите причину"></textarea></label>${reoffer}</section><div class="button-stack"><button class="danger-button" id="confirm-bank-outcome">Сохранить статус</button><button class="secondary-button" id="cancel-bank-outcome">Отмена</button></div>`)
  document.querySelector('#confirm-bank-outcome').addEventListener('click',async()=>{const reason=document.querySelector('#bank-outcome-reason').value.trim();if(reason.length<2)return toast('Укажите краткую причину');const button=document.querySelector('#confirm-bank-outcome');button.disabled=true;const payload={status,close_reason:reason};if(status==='not_opened')payload.reoffer_to_lead=document.querySelector('#bank-reoffer').checked;try{await api(`/api/lead-banks/${bankId}`,{method:'PATCH',body:JSON.stringify(payload)});toast(`${title}: сохранено`);await openLead(lead.id);await load()}catch(error){button.disabled=false;toast(error.message)}})
  document.querySelector('#cancel-bank-outcome').addEventListener('click',()=>openLead(lead.id))
}
...
function openPartner(id){
  ...
  document.querySelector('#partner-commission').value=p.commission
  ...
  document.querySelector('#save-commission').addEventListener('click',async()=>{try{const result=await api(`/api/partners/${id}`,{method:'PATCH',body:JSON.stringify({commission_percent:document.querySelector('#partner-commission').value.replace(',','.')})});toast(result.notification_error?'Процент сохранён, но уведомление не доставлено':'Процент сохранён');await load();openPartner(id)}catch(error){toast(error.message)}})
...
function updateCatalogPreview(){
  ...
  const partner=valid?Math.max(base-leadCost,0)*percent/100:null
  const team=valid?base-partner-leadCost:null
...
document.addEventListener('click',async event=>{ const lead=event.target.closest('[data-lead]'),partner=event.target.closest('[data-partner]'),channel=event.target.closest('[data-channel]'),catalogBank=event.target.closest('[data-catalog-bank]'),onlineHelp=event.target.closest('[data-online-help]'),bonusHelp=event.target.closest('[data-bonus-help]'),copyContact=event.target.closest('[data-copy-contact]'),telegramChat=event.target.closest('[data-telegram-chat]'),staff=event.target.closest('[data-staff]'),duplicate=event.target.closest('[data-duplicate]'),sheetLink=event.target.closest('[data-sheet-link]');try{if(onlineHelp){event.preventDefault();event.stopPropagation();showOnlineHelp(onlineHelp.dataset.onlineHelp);return}if(bonusHelp){event.preventDefault();event.stopPropagation();showBonusHelp(bonusHelp.dataset.bonusHelp);return}if(copyContact){event.preventDefault();await copyText(decodeURIComponent(copyContact.dataset.copyContact),'Скопировано');return}if(telegramChat){event.preventDefault();const url=telegramChat.dataset.telegramChat,tg=telegramWebApp();tg?.openTelegramLink?tg.openTelegramLink(url):window.open(url,'_blank','noopener');return}if(lead)await openLead(lead.dataset.lead);if(partner)openPartner(partner.dataset.partner);if(channel)openChannel(channel.dataset.channel);if(catalogBank)openCatalogBank(catalogBank.dataset.catalogBank);if(staff&&state.session.role==='admin'){await api(`/api/staff/${staff.dataset.staff}/toggle`,{method:'POST'});toast('Доступ изменён');await load()}if(duplicate)openDuplicate(duplicate.dataset.duplicate);if(sheetLink){const url=state.session?.[sheetLink.dataset.sheetLink];if(url){const tg=telegramWebApp();tg?.openLink?tg.openLink(url):window.open(url,'_blank','noopener')}}}catch(error){toast(error.message)} })
document.querySelectorAll('[data-screen]').forEach(x=>x.addEventListener('click',async()=>{showScreen(x.dataset.screen);if(state.session?.role==='partner'&&x.dataset.screen==='partners'){try{const data=await api(`/api/partner/cabinet?${partnerQuery()}`);state.partnerData=data;document.querySelector('#partner-commission-value').textContent=`${data.commission_percent}%`}catch(error){toast(error.message)}}}))
...
document.querySelectorAll('#admin-channel, #admin-period, #admin-lead-status').forEach(select=>select.addEventListener('change',()=>{renderAdminFilters();renderVisibleLeads()}))
...
document.addEventListener('visibilitychange',()=>{
  if(document.hidden||Date.now()-lastResumeRefresh<30000||!document.querySelector('#bottom-sheet').hidden)return
