...
function renderLeadCabinet(){
  ...
  const cards=state.leadBanks.map(item=>{const selectable=item.selected===null||(state.leadAddingBanks&&item.selected===false);const dimmed=item.selected===false&&!state.leadAddingBanks;const clientRefused=item.refusal_type==='client_refused';const bankRejected=item.refusal_type==='bank_rejected';const status=selectable?'Доступен для открытия':clientRefused?'':bankRejected?'Отказ банка':item.selected===false?'Не выбран':item.lead_payment_status==='paid'?bankLabels.lead_reward_paid:(bankLabels[item.status]||item.status);const statusTone=item.status==='opened'?'is-positive':bankRejected||item.status==='will_not_open'?'is-negative':'';return `<label class="client-bank-card ${selectable?'is-selectable':''} ${dimmed?'is-unselected':''}">${selectable?`<input class="bank-choice" type="checkbox" value="${esc(item.bank_id)}">`:''}${item.online_available?`<span class="online-badge">Можно онлайн <button type="button" data-online-help="${esc(item.online_help)}" aria-label="Условия открытия онлайн">${infoIcon}</button></span>`:''}<header><span class="client-bank-icon">${esc(initials(item.bank).slice(0,1)||'Б')}</span><div class="client-bank-copy"><h3>${esc(item.bank)}</h3>${status?`<p class="${statusTone}">${esc(status)}</p>`:''}</div>${selectable?'<span class="choice-mark">✓</span>':''}<strong class="bank-payout">${leadPayout(item)}</strong></header><section class="activation-action"><span>Условие активации</span><p>${esc(item.action_text||'Условие уточняется')}</p></section><small>Обновлено ${dateTime(item.updated)}</small></label>`}).join('')
...
function bankCard(item,employee,admin,quickActions){
  const financialEdit=admin?`<div class="field-row"><label class="field"><span>Общая ставка, прогноз</span><input type="number" data-estimate value="${item.income_estimate||''}"></label><label class="field"><span>Общая ставка, факт</span><input type="number" data-fact value="${item.income_fact||''}" ${item.status==='account_opened'&&!['confirmed','in_registry','paid'].includes(item.payment_status)?'':'disabled'}></label></div>`:''
  const decisionStatus={account_opened:'Счёт активирован',bank_rejected:'Отказ банка',client_refused:'Отказ клиента'}[item.status]||'Ожидается решение администратора'
  const decisionView=`<div class="value-row"><span>Статус</span><strong>${decisionStatus}</strong></div>${item.close_reason?`<div class="value-row"><span>Причина отказа</span><strong>${esc(item.close_reason)}</strong></div>`:''}`
  const adminEdit=`${decisionView}${financialEdit}<button class="secondary-button" data-save-bank="${item.id}">Сохранить финансовые данные</button>`
  const edit=admin?adminEdit:employee?decisionView:`<div class="value-row"><span>Статус</span><strong>${esc(bankLabels[item.status]||item.status)}</strong></div>`
  const quick=quickActions&&item.selected_by_lead===true&&!['account_opened','bank_rejected','client_refused'].includes(item.status)?`<div class="bank-quick-actions"><button class="primary-button" data-bank-opened="${item.id}">Счёт активирован</button><button class="danger-button" data-bank-reject="${item.id}" data-bank-name="${esc(item.bank)}">Отказ</button></div>`:''
...
function bindLeadActions(lead,admin){
  ...
  document.querySelectorAll('[data-bank-opened]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;const result=await api(`/api/lead-banks/${btn.dataset.bankOpened}`,{method:'PATCH',body:JSON.stringify({status:'account_opened'})});toast(result.sheet_sync_error||'Счёт отмечен как активированный');await openLead(lead.id);await load()}))
