...
function bankCard(item,employee,admin,quickActions){
  ...
  const reasonLabel=item.close_reason&&!['bank_rejected','client_refused','excluded'].includes(item.status)?'Причина предыдущего отказа':'Причина закрытия'
  const adminStatus={account_opened:'Счёт активирован',bank_rejected:'Отказ банка',client_refused:'Отказ клиента'}[item.status]||'Ожидается действие менеджера'
  const adminEdit=`<div class="value-row"><span>Статус</span><strong>${adminStatus}</strong></div>${financialEdit}${item.close_reason?`<div class="value-row"><span>Причина отказа</span><strong>${esc(item.close_reason)}</strong></div>`:''}<button class="secondary-button" data-save-bank="${item.id}">Сохранить финансовые данные</button>`
  const managerEdit=`<label class="field"><span>Статус</span><select data-bank-status>${options}</select></label><label class="field"><span>${reasonLabel}</span><input data-reason value="${esc(item.close_reason||'')}"></label><button class="secondary-button" data-save-bank="${item.id}">Сохранить изменения</button>`
  const edit=admin?adminEdit:employee?managerEdit:`<div class="value-row"><span>Статус</span><strong>${esc(bankLabels[item.status]||item.status)}</strong></div>`
...
function bindLeadActions(lead,admin){
  ...
  document.querySelectorAll('[data-save-bank]').forEach(btn=>btn.addEventListener('click',async()=>{const c=btn.closest('[data-bank-card]'),payload={},status=c.querySelector('[data-bank-status]'),reason=c.querySelector('[data-reason]'),estimate=c.querySelector('[data-estimate]'),fact=c.querySelector('[data-fact]');if(status)payload.status=status.value;if(reason)payload.close_reason=reason.value||null;if(estimate)payload.income_estimate=estimate.value||null;if(fact)payload.income_fact=fact.value||null;await api(`/api/lead-banks/${btn.dataset.saveBank}`,{method:'PATCH',body:JSON.stringify(payload)});toast('Изменения сохранены');await openLead(lead.id)}))
