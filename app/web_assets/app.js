...
function openPartner(id){
  ...
  openSheet(p.name,'Партнёр и каналы',`<section class="detail-section"><h3>Реферальные ссылки</h3>${links}</section><section class="form-card"><h3>Доступ партнёра</h3><div class="value-row"><span>Комиссия</span><strong>${esc(p.commission)}%</strong></div><label class="field"><span>Telegram ID</span><input id="partner-id" inputmode="numeric"></label><label class="field"><span>Username без @</span><input id="partner-user"></label><button class="primary-button inset-button" id="save-partner">Привязать кабинет</button></section>`)
  document.querySelector('#save-partner').addEventListener('click',async()=>{await api(`/api/partners/${id}/access`,{method:'PUT',body:JSON.stringify({telegram_id:document.querySelector('#partner-id').value,telegram_username:document.querySelector('#partner-user').value||null})});toast('Доступ настроен');closeSheet()})
}
