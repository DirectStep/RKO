...
function bankCard(item,employee,admin,quickActions){
  ...
  return `<article class="bank-card" data-bank-card="${item.id}">${online}<header><h4>${esc(item.bank)}</h4><span>${admin?money(item.team_profit_fact):esc(payLabels[item.payment_status]||item.payment_status)}</span></header>${clientView}${quick}${edit}${economics}${leadPayment}<div class="button-stack">${confirm}${next}</div></article>`
}
async function openLead(id){
  ...
  try{
    ...
    const canDeleteApplication=!lead.archived&&!hasActivatedAccounts&&!hasConfirmedPayments&&(admin||(managerRole&&lead.is_assigned_manager))
