...
function openPartner(id){
  ...
  openSheet(p.name,'Партнёр и каналы',`<section class="detail-section"><h3>Реферальные ссылки</h3>${links}</section><section class="form-card"><h3>Доступ партнёра</h3><div class="value-row"><span>Комиссия</span><strong>${esc(p.commission)}%</strong></div><div class="value-row"><span>Telegram</span><strong>${esc(telegram)}</strong></div><label class="field"><span>Telegram ID</span><input id="partner-id" inputmode="numeric" value="${esc(p.telegram_id||'')}" placeholder="Например, 123456789"></label><label class="field"><span>Username без @</span><input id="partner-user" value="${esc(username)}" placeholder="Например, gerasimov"></label><p class="field-note">Username сам по себе не даёт доступ. Для входа партнёра нужен его Telegram ID.</p><button class="primary-button inset-button" id="save-partner">Сохранить доступ</button></section><section class="destructive-section"><button class="danger-button" id="show-remove-partner">Убрать партнёра</button><div id="remove-partner-confirm" hidden><p>Партнёр и его каналы будут удалены. Если уже есть заявки, удаление не выполнится.</p><div class="button-stack"><button class="danger-button" id="remove-partner">Да, удалить</button><button class="secondary-button" id="cancel-remove-partner">Отмена</button></div></div></section>`)
  document.querySelector('#save-partner').addEventListener('click',async()=>{await api(`/api/partners/${id}/access`,{method:'PUT',body:JSON.stringify({telegram_id:document.querySelector('#partner-id').value,telegram_username:document.querySelector('#partner-user').value||null})});toast('Доступ настроен');await load();openPartner(id)})
