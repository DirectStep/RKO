...
function bankCard(item,employee,admin,quickActions){
  ...
  const decisionStatus={account_opened:'Счёт активирован',bank_rejected:'Отказ банка',client_refused:'Отказ клиента'}[item.status]||'Ожидается действие менеджера'
  ...
  const quick=quickActions&&!['account_opened','bank_rejected','client_refused'].includes(item.status)?`<div class="bank-quick-actions"><button class="primary-button" data-bank-opened="${item.id}">Счёт активирован</button><button class="danger-button" data-bank-reject="${item.id}" data-bank-name="${esc(item.bank)}">Отказ</button></div>`:''
