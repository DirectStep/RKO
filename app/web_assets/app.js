...
const clientWorkflowLabels = { awaiting_admin:'Заявка принята',admin_processing:'Подбираем подходящие банки',awaiting_client_selection:'Выбери банки',awaiting_manager:'Ожидаем менеджера',manager_processing:'Менеджер сопровождает заявку',not_eligible:'Пока не сможем помочь' }
...
function renderAdminFilters(){
  ...
  channelSelect.innerHTML=partnerId?'<option value="">Все каналы партнёра</option>'+channels.map(channel=>`<option value="${channel.id}">${esc(channel.name)}</option>`).join(''):'<option value="">Сначала выбери партнёра</option>'
...
function renderLeadCabinet(){
  ...
  const cards=state.leadBanks.map(item=>`<label class="client-bank-card ${canSelect?'is-selectable':''}">${canSelect?`<input class="bank-choice" type="checkbox" value="${esc(item.bank_id)}">`:''}${item.online_available?`<span class="online-badge">Можно онлайн <button type="button" data-online-help="${esc(item.online_help)}" aria-label="Условия открытия онлайн">?</button></span>`:''}<header><span class="client-bank-icon">${esc(initials(item.bank).slice(0,1)||'Б')}</span><div class="client-bank-copy"><h3>${esc(item.bank)}</h3><p>${canSelect?'Доступен для открытия':esc(bankLabels[item.status]||item.status)}</p></div>${canSelect?'<span class="choice-mark">✓</span>':''}<strong class="bank-payout">${money(item.lead_payout)}</strong></header><section class="activation-action"><span>Условие активации</span><p>${esc(item.action_text||'Условие уточняется')}</p></section><small>Обновлено ${dateTime(item.updated)}</small></label>`).join('')
...
function bankCard(item,employee,admin,quickActions){
  ...
  const quick=quickActions&&!['account_opened','bank_rejected','client_refused','excluded'].includes(item.status)?`<div class="bank-quick-actions"><button class="primary-button" data-bank-opened="${item.id}">Счёт открыт</button><button class="danger-button" data-bank-reject="${item.id}" data-bank-name="${esc(item.bank)}">Отказ</button></div>`:''
...
async function openLead(id){
  ...
  try{
    ...
    const canPublishBanks=!lead.archived&&admin&&lead.is_primary_admin&&['admin_processing','awaiting_client_selection'].includes(lead.workflow_stage)
    ...
    const workflowActions=!lead.archived&&(admin&&lead.workflow_stage==='awaiting_admin'?'<button class="primary-button inset-button" id="claim-admin">Взять в первичную работу</button>':managerRole&&lead.workflow_stage==='awaiting_manager'?'<button class="primary-button inset-button" id="claim-manager">Взять в сопровождение</button>':'')
    ...
    const publishBanksAction=canPublishBanks?`<button class="primary-button inset-button" id="publish-banks"${lead.banks.length?'':' disabled'}>Открыть банки клиенту</button>`:''
...
function openPartnerLead(id){
  ...
  const banks=lead.banks.map((bank,index)=>`<button type="button" class="bank-card bank-card-button" data-partner-bank="${index}">${bank.online_available?`<span class="online-badge compact">Можно онлайн <span role="button" tabindex="0" data-online-help="${esc(bank.online_help)}" aria-label="Условия открытия онлайн">?</span></span>`:''}<header><h4>${esc(bank.bank)}</h4><span>${money(bank.reward_estimate)}</span></header><section class="activation-action"><span>Условие активации</span><p>${esc(bank.action_text||'Условие уточняется')}</p></section><small>Нажми, чтобы посмотреть выплаты</small></button>`).join('')
  openSheet(lead.name,`${lead.short_id} · ${lead.is_repeat?'Повторная · ':''}${leadLabels[lead.status]||lead.status}`,`<section class="detail-section"><h3>Заявка клиента</h3><div class="value-row"><span>Номер</span><strong>${esc(lead.short_id)}</strong></div><div class="value-row"><span>Дата заявки</span><strong>${dateTime(lead.date)}</strong></div><div class="value-row"><span>Последнее обновление</span><strong>${dateTime(lead.updated)}</strong></div><div class="value-row"><span>Канал</span><strong>${esc(lead.channel)}</strong></div><div class="value-row"><span>Статус</span><strong>${esc(leadLabels[lead.status]||lead.status)}</strong></div>${telegram}${admin}</section><section class="detail-section"><h3>Банки и выплаты</h3><div class="value-row"><span>Открыто / в работе / запланировано</span><strong>${counts.opened||0} / ${counts.in_progress||0} / ${counts.planned||0}</strong></div><div class="value-row"><span>Не будет открыто</span><strong>${counts.will_not_open||0}</strong></div><div class="value-row"><span>Ожидаемая выплата</span><strong>${money(lead.reward_estimate)}</strong></div><div class="value-row"><span>Подтверждённая выплата</span><strong>${money(lead.reward_fact)}</strong></div></section>${banks||'<p class="empty">Банки пока не добавлены</p>'}`)
...
function openPartnerBank(lead,index){
  ...
  openSheet(bank.bank,'Ставки по заявке',`<section class="detail-section"><div class="value-row"><span>Статус счёта</span><strong>${esc(bankLabels[bank.status]||bank.status)}</strong></div><div class="value-row"><span>Твоя ожидаемая выплата</span><strong>${money(bank.reward_estimate)}</strong></div><div class="value-row"><span>Подтверждено тебе</span><strong>${money(bank.reward_fact)}</strong></div><div class="value-row"><span>Выплата клиенту</span><strong>${money(bank.lead_reward_estimate)}</strong></div><div class="value-row"><span>Можно открыть онлайн</span><strong>${String(bank.online_text||'').toLowerCase().startsWith('да')?'Да':'Нет'}</strong></div></section><section class="activation-action"><span>Условие активации</span><p>${esc(bank.action_text||'Условие уточняется')}</p></section><button class="secondary-button" id="back-to-partner-lead">Назад к заявке</button>`)
...
function bindLeadActions(lead,admin){
  ...
  document.querySelectorAll('[data-bank-opened]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;await api(`/api/lead-banks/${btn.dataset.bankOpened}`,{method:'PATCH',body:JSON.stringify({status:'account_opened'})});toast('Счёт отмечен как открытый');await openLead(lead.id);await load()}))
...
function openPartner(id){
  ...
  const activation=p.activated?'<div class="value-row"><span>Партнёрский кабинет</span><strong>Уже активирован</strong></div>':'<p class="field-note">Создай одноразовую ссылку и отправь её партнёру. Новая ссылка отменяет предыдущую.</p><button class="primary-button inset-button" id="create-partner-activation-link">Создать ссылку активации</button><div id="partner-activation-result" hidden></div>'
  ...
  document.querySelector('#create-partner-activation-link')?.addEventListener('click',async()=>{const button=document.querySelector('#create-partner-activation-link');button.disabled=true;try{const result=await api(`/api/partners/${id}/activation-link`,{method:'POST',body:'{}'}),target=document.querySelector('#partner-activation-result');target.hidden=false;target.innerHTML=`<div class="value-row"><span>Одноразовая ссылка</span><strong>${esc(result.link)}</strong></div><button class="secondary-button inset-button" id="copy-partner-activation-link">Скопировать ссылку</button>`;button.textContent='Создать новую ссылку';document.querySelector('#copy-partner-activation-link').addEventListener('click',()=>copyText(result.link))}catch(error){toast(error.message)}finally{button.disabled=false}})
...
function openNewPartner(){
  openSheet('Новый партнёр','Доступ и процент',`<section class="form-card"><label class="field"><span>Название или имя партнёра</span><input id="new-partner-name" maxlength="160" placeholder="Например, Максим"></label><label class="field"><span>Процент партнёра</span><input id="new-partner-percent" inputmode="decimal" value="20"></label><label class="field"><span>Telegram username без @</span><input id="new-partner-username" placeholder="Можно оставить пустым"></label><p class="field-note">Ты автоматически станешь ответственным администратором этого партнёра.</p></section><button class="primary-button" id="create-partner">Добавить партнёра</button>`)
  document.querySelector('#create-partner').addEventListener('click',async()=>{const name=document.querySelector('#new-partner-name').value.trim(),commission=document.querySelector('#new-partner-percent').value.replace(',','.'),telegram_username=document.querySelector('#new-partner-username').value.trim()||null;if(name.length<2)return toast('Укажи имя партнёра');await api('/api/partners',{method:'POST',body:JSON.stringify({name,commission_percent:commission,telegram_username})});toast('Партнёр добавлен');closeSheet();await load();showScreen('partners')})
}
function openChannel(id){
  const channel=state.channels.find(item=>item.id===id)
  if(!channel)return toast('Канал не найден')
  openSheet(channel.name,channel.active?'Канал работает':'Канал отключён',`<section class="detail-section"><div class="value-row"><span>Реферальная ссылка</span><strong>${esc(channel.link)}</strong></div></section><div class="button-stack"><button class="primary-button" id="copy-channel-link">Скопировать ссылку</button><button class="danger-button" id="remove-channel">Удалить канал</button><button class="secondary-button" id="cancel-channel">Закрыть</button></div>`)
  document.querySelector('#copy-channel-link').addEventListener('click',()=>copyText(channel.link))
