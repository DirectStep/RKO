...
async def open_admin_menu(message: Message, database: Database, settings: Settings) -> None:
...
async def admin_home(callback: CallbackQuery, database: Database, settings: Settings) -> None:
...
async def admin_lead(callback: CallbackQuery, database: Database, settings: Settings) -> None:
    ...
    if lead is None:
        ...
        return
    if isinstance(callback.message, Message):
        await callback.message.edit_text(format_lead(lead), reply_markup=admin_back_keyboard())
    await callback.answer()
...
def format_lead(lead: Lead) -> str:
    ...
    return (
        ...
        f"Источник: {lead.assignment_status.value}\n\n"
        f"Анкета:\n{answers}"
    )
