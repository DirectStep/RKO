...
function renderLeadCabinet(){
  ...
  const cards=state.leadBanks.map(item=>{const selectable=item.selected===null||(state.leadAddingBanks&&item.selected===false);const dimmed=item.selected===false&&!state.leadAddingBanks;const displayStatus=item.display_status||item.status;const status=item.selected===null?'Доступен для открытия':item.selected===false&&displayStatus==='planned'?'Не выбран':bankLabels[displayStatus]||displayStatus;const statusTone=displayStatus==='account_opened'||displayStatus==='lead_reward_paid'?'is-positive':['not_opened','bank_rejected','client_refused','cut','duplicate'].includes(displayStatus)?'is-negative':'';return `<label class="client-bank-card ${selectable?'is-selectable':''} ${dimmed?'is-unselected':''}">${selectable?`<input class="bank-choice" type="checkbox" value="${esc(item.bank_id)}">`:''}${item.online_available?`<span class="online-badge">Можно онлайн <button type="button" data-online-help="${esc(item.online_help)}" aria-label="Условия открытия онлайн">${infoIcon}</button></span>`:''}<header><span class="client-bank-icon">${esc(initials(item.bank).slice(0,1)||'Б')}</span><div class="client-bank-copy"><h3>${esc(item.bank)}</h3>${status?`<p class="${statusTone}">${esc(status)}</p>`:''}</div>${selectable?'<span class="choice-mark">✓</span>':''}<strong class="bank-payout">${leadPayout(item)}</strong></header><section class="activation-action"><span>Условие активации</span><p>${esc(item.action_text||'Условие уточняется')}</p></section><small>Обновлено ${dateTime(item.updated)}</small></label>`}).join('')
...
function bankCard(item,employee,admin,quickActions){
  const financialEditable=admin&&!['not_opened','bank_rejected','client_refused','cut','duplicate'].includes(item.status)
...
function openPartnerLead(id){
  ...
  const banks=lead.banks.map((bank,index)=>`<button type="button" class="bank-card bank-card-button" data-partner-bank="${index}"><header><h4>${esc(bank.bank)}</h4><span>${money(bank.reward_estimate)}</span></header><small>Нажмите, чтобы посмотреть выплаты</small></button>`).join('')
